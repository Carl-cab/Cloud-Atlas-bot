-- First, check what valid statuses are allowed in system_health
-- and then create test data with valid statuses only
...
BEGIN
  ...
  -- Check for performance degradation (only for 'warning' status)
  ELSIF NEW.status = 'unhealthy' AND NEW.response_time_ms > 1000 THEN
    PERFORM public.create_system_alert(
...
-- Create comprehensive test data for system health using only valid status values
...
('ML Engine', 'unhealthy', 450, 'High response time detected'),
...
('WebSocket Service', 'unhealthy', 289, 'Connection timeout'),
