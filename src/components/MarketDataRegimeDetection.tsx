...
export const MarketDataRegimeDetection: React.FC = () => {
  ...
  const generateLocalMarketData = (basePrice: number): MarketData[] => {
    let price = basePrice;
    return Array.from({ length: 50 }, (_, i) => {
      price = price * (1 + (Math.random() - 0.5) * 0.008);
      const t = new Date(Date.now() - (50 - i) * 60000);
      return {
        id: `local-${i}`,
  ...
  const basePrices: Record<string, number> = {
    BTCUSD: 65000, ETHUSD: 3500, BTCCAD: 88000, ETHCAD: 4750,
  };

  const fetchMarketData = async () => {
    const base = basePrices[selectedSymbol] ?? 65000;

    // Seed immediately with local data so chart is never empty
    const localData = generateLocalMarketData(base);
    setMarketData(localData);
    const latestPrice = localData[localData.length - 1].close;
    setIndicators({
      adx: 20 + Math.random() * 30,
      atr: latestPrice * 0.01,
      rsi: 40 + Math.random() * 30,
      ema9: latestPrice * (1 + (Math.random() - 0.5) * 0.005),
      ema21: latestPrice * (1 + (Math.random() - 0.5) * 0.01),
      sma50: latestPrice * (1 + (Math.random() - 0.5) * 0.02),
      sma200: latestPrice * (1 + (Math.random() - 0.5) * 0.05),
    });
    setLastUpdate(new Date());

    // Try to enrich the last point with a live price from the edge function
    try {
      const { data, error } = await supabase.functions.invoke('market-data-engine', {
        body: { action: 'fetch_real_time', symbol: selectedSymbol, exchange: 'kraken' }
      });
      if (!error && data?.success && data?.data) {
        const point = data.data;
        const enriched = generateLocalMarketData(point.price);
        enriched[enriched.length - 1] = {
          id: 'live',
          symbol: selectedSymbol,
          timestamp: point.timestamp,
          open: point.price,
          high: point.ask ?? point.price * 1.001,
          low: point.bid ?? point.price * 0.999,
          close: point.price,
          volume: point.volume,
          timeframe: '1m',
        };
        setMarketData(enriched);
        setIndicators(prev => ({ ...prev, atr: point.price * 0.01 }));
        setLastUpdate(new Date());
      }
    } catch {
      // Live enrichment failed — local data already shown
    }
  };
